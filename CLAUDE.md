...
## 核心结论逐步复现代码（答辩/审查用，原始记录性质）

`原始数据分析代码/结论一_总体再手术率两种术式无差异/` 下 step1–step4，
均从原始 xlsx 现算（不引用稿件已算好的数字），逐条核对与 Table 3 上半部分
（总体率一行 + Firth 三行）一致：

- step1_数据准备与分组计数.py：三组人数与再手术例数
- step2_风险比.py：Fisher 精确检验 + OR 条件精确（Cornfield）区间
- step3_风险差.py：风险差 + Newcombe 区间
- step4_Firth校正.py：Firth 惩罚似然逻辑回归，未校正/+新生儿+坏死/+年龄段+坏死 三行

`原始数据分析代码/结论二_再手术病因构成因术式而异/` 下 step1–step4，
逐条核对与 Table 2 一致（六病因分组计数、逐病因 Fisher+OR/NE 判定、
Holm 六项族内校正、十二指肠梗阻风险差 RD +4.7pp 95% CI 1.9–8.0）。

**2026-09-23 路径定位改为不限层数、逐级向上搜索**（原先 `sys.path.insert(0,
dirname(dirname(...)))` 固定翻两层，结论二在仓库里的嵌套层级
`原始数据分析代码/结论N_.../stepN.py` 是两层，需要翻三层才到仓库根目录，
暴露了这个硬编码之前只是被结论一测试时用的 `PYTHONPATH` 环境变量掩盖住、
没真正验证过）。现改为从脚本自身目录开始逐级 `dirname` 向上找，直到找到
含 `_dataprep.py` 的目录为止，不管脚本放进几层子文件夹都能定位——作者
本机把这类代码放进 `③肠旋转不良术后再手术\Python分析代码\`（一层）也仍适用。
每个结论文件夹下配一个纯 ASCII 的 `运行全部步骤.bat`（`cd /d %~dp0` 定位
自身目录 + `stepN*.py` 通配符调用，不写死中文文件名，避免 cmd.exe 代码页
不一致导致乱码/找不到文件），双击即可依次跑完该结论的全部 step。

`make_conclusion1_table.py` 把 step1–4 的实测结果整理成三线表（Word，仅顶线/
表头下线/底线，无竖线），产出 `核心结论一_三线表.docx`。后续结论如需同款
表格，复制该脚本改 `ROWS`/`HEADER`/`TITLE` 即可。
