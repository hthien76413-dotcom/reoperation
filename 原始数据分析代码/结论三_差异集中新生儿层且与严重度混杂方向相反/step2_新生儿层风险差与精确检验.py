import math
import os, sys
from scipy import stats
_d = os.path.dirname(os.path.abspath(__file__))
while not os.path.exists(os.path.join(_d, "_dataprep.py")):
    _p = os.path.dirname(_d)
    if _p == _d:
        raise SystemExit("找不到 _dataprep.py，请确认本脚本仍在原仓库目录树下（任意层子文件夹均可）")
    _d = _p
sys.path.insert(0, _d)
from _dataprep import load, approach, reop_set, adjudicated

D = load()
R = reop_set()
A = adjudicated()
malrot, first = D["malrot"], D["first"]
R = {p for p in R if p in set(malrot)}
NEO = D["neonate"]

LAP = {p for p in malrot if approach(first[p]) == "腹腔镜完成"}
OPR = {p for p in malrot if approach(first[p]) != "腹腔镜完成"}
lap_neo, opr_neo = LAP & NEO, OPR & NEO

k1 = sum(1 for p in lap_neo if p in R and A[p]["cause"] == "十二指肠持续梗阻")
n1 = len(lap_neo)
k2 = sum(1 for p in opr_neo if p in R and A[p]["cause"] == "十二指肠持续梗阻")
n2 = len(opr_neo)

pv = stats.fisher_exact([[k1, n1 - k1], [k2, n2 - k2]])[1]
p1, p2 = k1 / n1, k2 / n2
rd = p1 - p2


def wilson(k, n, z=1.96):
    p = k / n; den = 1 + z * z / n
    c = (p + z * z / (2 * n)) / den
    h = z * math.sqrt(p * (1 - p) / n + z * z / (4 * n * n)) / den
    return c - h, c + h


def newcombe(k1, n1, k2, n2, z=1.96):
    l1, u1 = wilson(k1, n1, z)
    l2, u2 = wilson(k2, n2, z)
    p1, p2 = k1 / n1, k2 / n2
    lo = (p1 - p2) - z * math.sqrt(l1 * (1 - l1) / n1 + u2 * (1 - u2) / n2)
    hi = (p1 - p2) + z * math.sqrt(u1 * (1 - u1) / n1 + l2 * (1 - l2) / n2)
    return lo, hi


lo, hi = newcombe(k1, n1, k2, n2)
nnh = 1 / rd if rd > 0 else float("inf")

print("新生儿层十二指肠持续梗阻:")
print("腹腔镜完成: %d/%d = %.4f" % (k1, n1, p1))
print("开腹相关:   %d/%d = %.4f" % (k2, n2, p2))
print("Fisher exact p = %.6f" % pv)
print("风险差 (RD) = %.4f (即 %+.2f 个百分点)" % (rd, rd * 100))
print("95%% CI (Newcombe) = %.4f 到 %.4f (即 %+.2f 到 %+.2f 个百分点)"
      % (lo, hi, lo * 100, hi * 100))
print("对照组事件数 c = %d -> 零单元，OR 不可估" % k2)
print("Number needed to harm (NNH) = %.1f" % nnh)
